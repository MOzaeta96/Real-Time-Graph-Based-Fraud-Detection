...
SELECT
  ...
  COALESCE(MAX(rt.amount), 0)::double precision AS max_amount,
  CASE
    WHEN AVG(rt.amount) = 0 THEN 0
    ELSE MAX(rt.amount) / AVG(rt.amount)
  END::double precision AS amount_spike_ratio,
  ...
  (
    SUM(CASE WHEN rt.is_fraud = 1 THEN 1 ELSE 0 END)::double precision
...
SELECT
  device_id,
  COUNT(*)::int AS device_user_degree
FROM user_device_edges
GROUP BY device_id;

-- device -> fraud rate on that device
DROP TABLE IF EXISTS tmp_device_fraud_rate;
CREATE TEMP TABLE tmp_device_fraud_rate AS
SELECT
  device_id,
  (
    SUM(CASE WHEN is_fraud = 1 THEN 1 ELSE 0 END)::double precision
    / COUNT(*)::double precision
  ) AS device_fraud_rate
FROM raw_transactions
GROUP BY device_id;
...
SELECT
  merchant_id,
  COUNT(*)::int AS merchant_device_degree
...
SELECT
  ...
  -- device fraud pressure
  COALESCE(udf.avg_device_fraud_rate, 0)::double precision AS avg_device_fraud_rate,
  COALESCE(udf.max_device_fraud_rate, 0)::double precision AS max_device_fraud_rate,
...
LEFT JOIN (
  SELECT
    user_id,
    COUNT(*)::int AS user_device_degree
  FROM user_device_edges
  GROUP BY user_id
...
LEFT JOIN (
  SELECT
    user_id,
    COUNT(*)::int AS user_merchant_degree
...
LEFT JOIN (
  ...
  JOIN tmp_device_user_degree dud
    ON dud.device_id = ude.device_id
...
LEFT JOIN (
  SELECT
    ude.user_id,
    AVG(dfr.device_fraud_rate)::double precision AS avg_device_fraud_rate,
    MAX(dfr.device_fraud_rate)::double precision AS max_device_fraud_rate
  FROM user_device_edges ude
  JOIN tmp_device_fraud_rate dfr
    ON dfr.device_id = ude.device_id
  GROUP BY ude.user_id
) udf ON udf.user_id = u.user_id
...
LEFT JOIN (
  ...
  JOIN tmp_merchant_device_degree mdd
    ON mdd.merchant_id = ume.merchant_id
...
SET
  ...
  avg_device_fraud_rate = g.avg_device_fraud_rate,
  max_device_fraud_rate = g.max_device_fraud_rate,
